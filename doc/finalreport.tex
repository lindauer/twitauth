...
As a matter of convention, we classified each tweet as either a positive or negative by determining the maximum value of each linear classifier method for a single tweet, setting that classifer to one and the others to zero.  We used the frequency of a classifier being chosen across the sample set as an aggregate to determine confidence accross a sampling of tweets.  However, this result did not vary notably from using the sum of the raw values produced by each tweet.
  
Similar to the Narayanan paper, we discovered that the resulting training error of this One Vs. All (OWA) method, ranged between 20 and 30 percent which was an order of magnitude worse than our NN classifier.  This is a known problem for OWA  classifiers with a large number of labels.  For each label in our data set, the RLSC generally had a thousand positive examples compared to nearly half a million negative examples. In order to address this issue we explored a handful of methods to level the data as the algorithm.  Rescaling the data was not an option as the algorithm is invariant to rescaling of the examples.  We implemented several methods \cite{Schapire} to attempt to reduce this error by including pruning \cite{Dekel} negative examples for each OWA being trained and artifically creating like samples for training.  However, we found that these methods only improved the classification error by only $5-7\%$.  

Eventually we derived the following closed form solution for appropriatley balancing the ratio of positive and negative examples inspired by the concept described in the Narayanan paper. We let $\Phi_j$ be a diagonal matrix where $\Phi_{j_{ii}}$ is equal to the ratio of negative to positive examples for the current classifier j.

$$J(\theta) = 0.5 (X\theta - y)^T \Phi_j (X\theta-y) + \lambda ||\theta||^2 $$
$$ \bigtriangledown_\theta J(\theta) = X^T \Phi_j X \theta -X^T \Phi_j y +2 \lambda \theta = 0$$
$$ \theta= (X^T\Phi_j X+2 \lambda I)^{-1} X^T \Phi_j y$$

Using this closed form required the generation of $\Phi \in \Re^{500k x 500k}$   for each of the 844 classifiers and required us to return to iterating through calculating each classifier independently.  Due to the memory requirements of this algorithm in MATLAB using sparse matrices, this algorithm was difficult to parallize beyond a handful of labs and thus still required several hours to complete.  However, with this results we were able to reduce our training error down to $1.1\%$ and our generalization error to $4.7\%$.  Most importantly,  the algorithm performed significantly better on the set of known twitter account pairs with the same author, classifying 41% of them correctly.  

In accomplishing the above results, we did not experiment much with the regularization paramater $\lambda$.  Our choice of lambda was the minimum order of magnitude that did not result in the classifiers calculation resulting in nearly singular matrices for the algorithm, which, in this case was $10^{-9}$.  We minimally explored methods to improve the performance and memory requirements of the algorithm, but did not explore methods such as conjugate descent and dynamic programed as referenced by Naranayan and Rifkin \cite{Rifkin}. 

\section{Conclusion}

We are encouraged by our initial result. The algorithms both exhibits
excellent accuracy in cross validation, and even do fairly well in
